...
Lists, sets, tuples and dictionaries are some of the basic data structures in python. These are basically ways of storing multiple values behind a single variable. If a python object is immutable it means you can't change it after you've created it, e.g. you can't remove/add new items to a `tuple` but you can remove/add them to a `list` or `dict` etc.. Lists can contain multiple types of variables, including other lists. e.g. (ignore >>> you don't have to type it in, it's just copy pasted from the shell)
...
>>>
```
if you request an item that's not in the list it'll give you an error
