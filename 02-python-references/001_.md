...
Lists, sets, tuples and dictionaries are some of the basic data structures in python. These are basically ways of storing multiple values behind a single variable. If a python object is immutable it means you can't change it after you've created it, e.g. you can't remove/add new items to a `tuple` but you can remove/add them to a `list` or `dict` etc.. Lists can contain multiple types of variables, including other lists. e.g.
...
With square bracket notation you create lists, and you separate values contained in it by comma. With regular bracket notation e.g. `name_of_tuple = (1, 2, 3, "orange", 3.4)` you create tuples. Each one of the itmes in a list is assigned an index by default. The first item is at index number 0, e.g. 
